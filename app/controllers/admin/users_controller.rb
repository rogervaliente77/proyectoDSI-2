module Admin
  class UsersController < ApplicationController
    ...
    # Crear usuario desde super_admin
    def create
    ...
    # Se puede llamar desde un botón en el portal para activar allow_notifications
    def enable_notifications
      @user = User.find(params[:id])
      if @user.update(allow_notifications: true)
        render json: { status: 'success', message: 'Notificaciones activadas' }
      else
        render json: { status: 'error', message: 'No se pudo activar notificaciones' }, status: :unprocessable_entity
      end
    end
    ...
    def user_params
      params.require(:user).permit(
        :is_valid, :first_name, :last_name, :role, :password, :password_confirmation, 
        :email, :role_id, :allow_notifications # <-- Permitimos editar este campo
      )
    end
